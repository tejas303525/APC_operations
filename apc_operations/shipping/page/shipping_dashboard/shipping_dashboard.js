frappe.pages['shipping-dashboard'].on_page_load = function (wrapper) {
...
frappe.pages['shipping-dashboard'].on_page_show 	= function () {
	...
	_render_job_orders(rows) {
		...
		rows.forEach(c => {
			...
			const jo_link = c.job_order
				? `<a href="/app/job-order/${c.job_order}">${c.job_order}</a>`
