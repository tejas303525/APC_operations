    ...
    refresh(frm) {
        ...
        if (frm.doc.security_draft_delivery_note) {
            ...
            frm.add_custom_button(__('Print Draft Delivery Note'), () => {
                const url = `/printview?doctype=${encodeURIComponent('Security Draft Delivery Note')}&name=${encodeURIComponent(frm.doc.security_draft_delivery_note)}&format=${encodeURIComponent('Draft Delivery Note')}&no_letterhead=0&_lang=en`;
